import React from 'react';

export interface LemmoIconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
}

/**
 * LEMMO solid brand mark (3 circles constellation)
 * Single source brand mark from System_design / ui
 */
export function LemmoMark({ size = 24, className, ...props }: LemmoIconProps) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 512 512"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      {...props}
    >
      <g clipPath="url(#clip0_lemmo_mark)">
        <path
          d="M143.836 311.343C174.673 311.343 199.671 336.341 199.671 367.178C199.671 398.015 174.673 423.014 143.836 423.014C112.998 423.014 88 398.015 88 367.178C88 336.341 112.998 311.343 143.836 311.343Z"
          fill="currentColor"
        />
        <path
          d="M367.178 311.343C398.015 311.343 423.014 336.341 423.014 367.178C423.014 398.015 398.015 423.014 367.178 423.014C336.341 423.014 311.343 398.015 311.343 367.178C311.343 336.341 336.341 311.343 367.178 311.343Z"
          fill="currentColor"
        />
        <path
          d="M255.507 88C286.344 88 311.343 112.998 311.343 143.836C311.343 174.673 286.344 199.671 255.507 199.671C224.67 199.671 199.671 174.673 199.671 143.836C199.671 112.998 224.67 88 255.507 88Z"
          fill="currentColor"
        />
      </g>
      <defs>
        <clipPath id="clip0_lemmo_mark">
          <rect width="512" height="512" fill="white" />
        </clipPath>
      </defs>
    </svg>
  );
}

/**
 * LEMMO App Icon (Lime badge with dark mark)
 */
export function LemmoAppIcon({ size = 32, className, ...props }: LemmoIconProps) {
  return (
    <div
      style={{
        width: typeof size === 'number' ? `${size}px` : size,
        height: typeof size === 'number' ? `${size}px` : size,
        borderRadius: 'var(--lemu-radius-150)',
        background: 'var(--lemu-color-surface-brand)',
        color: 'var(--lemu-color-font-on-brand)',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        boxShadow: 'var(--lemu-shadow-glow-brand)',
        flexShrink: 0,
      }}
      className={className}
    >
      <LemmoMark size="65%" />
    </div>
  );
}
